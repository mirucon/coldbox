...
jQuery(function ($) {
  ...
  $('a[href*="#"]').not('.noscroll').not('a[href^="#comment-list"]').not('a[href^="#ping-list"]').click(function () {
    ...
    if (hrefPageUrl === currentUrl) { // If the link goes to current page
      ...
      if (window.matchMedia('(min-width: 767px)').matches) {
        if ($wpadminbar.css('position') === 'fixed') { // When the admin bar is shown.
          position -= $wpadminbar.outerHeight()
        }
        if ($body.hasClass('sticky-header') && $body.hasClass('header-row')) { // When the header is sticky.
  ...
  $('.search-toggle, .close-toggle').not('.search-form').click(function () {
    ...
    searchCount++
    if (searchCount % 2 === 0) { $body.addClass('modal-search-closed') }
    if (searchCount % 2 === 1) { $body.removeClass('modal-search-closed') }
  })
  ...
  $navToggle.click(function () {
    $(this).toggleClass('open')
    navCount++
    if (navCount % 2 === 0) { $body.addClass('header-menu-closed') }
    if (navCount % 2 === 1) { $body.removeClass('header-menu-closed') }
  })
